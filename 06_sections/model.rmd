...
We can use the model described above to produce a covariance structure in shootings. Let $a_{ij} = \Cov [ v_i^t, v_j^t ]$ denote the covariance in violence between territories $i$ and $j$. Proposition `r Pcov` describes the $a_{ij}$ for which a derivation can be found in Appendix `r Apdx_cov`.
