...
- @Sanchez1991, @Levitt2000, @Papachristos2009, @Papachristos2013
...
- Natural Language Processing: @Sobrino2019 
- Mixed methods: Alcocer et al, Mapping Criminal Organizations

## Approach

**Observables:** Time-stamped, geolocated shootings
...
- **Assumption `r Aintragang_nz`:** $\mathbb{E} [ \xi_k^t ] > 0$ for all $k \neq 0$ and $\xi_0^t = 0$ for all $t$.
- **Assumption `r Aintragang_iid`:** $\mathbb{E} [ \xi_k^t \xi_{\ell}^t ]] - \mathbb{E} [ \xi_k^t ] \mathbb{E}[ \xi_{\ell}^t ] = 0$ for all $k \neq \ell$
...
- **Assumption `r Aintergang_iid`:** $\mathbb{E} \left[ \epsilon_{k, \ell}^t \epsilon_{m, n}^t \right] - \mathbb{E} \left[ \epsilon_{k, \ell}^t  \right] \mathbb{E} \left[ \epsilon_{m, n}^t \right] = 0$ for $m, n \notin \left\{ k, \ell \right\}$.
...
\frac{m_{\pi(i)}}{n_{\pi(j)}} \frac{m_{\pi(j)}}{n_{\pi(i)}} \frac{c_{\pi(j)}}{c_{\pi(i)}} \text{Var} [\epsilon_{\pi(i), \pi(j)}^t] & \text{if } \pi(i) \neq \pi(j) \\
0 & \text{otherwise}
\end{cases}
$$
...
- $\Theta_{N \times K + 1} = (\theta_{ik})_{\left\{ i \in \mathcal{N}, k \in \mathcal{N} \right\} }$ (membership matrix, $\theta_{ik} = 1$ if $\pi(i) = k$ and $0$ otherwise)
